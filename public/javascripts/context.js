VRL.TheContext = function (docWidth, docHeight, extraSpaces) {
	"use strict";
	// default settings
	var margin = {
		top : 0,
	...
	theContext.addObserver = function addObserver(newObserver) {
		subject.observe(newObserver);
	};

	theContext.removeObserver = function removeObserver(deleteObserver) {
		subject.unobserve(deleteObserver);
	};
	
	theContext.handlesUpdated = function fetchRange() {
		// notify our observers 
		subject.notify(theRange, "theContext");
	};
	//##############################################
	
	
	function theContext(selection) {
		selection.each(function (data) {
			...
			data.forEach(function (d) {
				...
				//log("theContext: d = " + d);
				d.map(function(dd) {
					//log("theContext: dd = " + dd.timestamp);
					xDomain.push(new Date(dd.timestamp - 0).getTime());
				});
				
			});
			//log("theContext: xDomain = " + xDomain);
			/**/
			...
			//log("theContext: start date :" + xDomain[0]);
			//log("theContext: Stop date :" + xDomain[xDomain.length - 1]);
			...
			if(xDomain[0] + (1000*60*60*24) > xDomain[xDomain.length - 1]) {
				log("theContext:  range smaller than a day");
			...
			*/
			...
			//log("theContext: yValues : " + yValues);
			...
			context.append("g")
					.attr("id", "theContextObjectContainer");
					
					
			//#####################################################################################
			//### Drawing each of the data (except GPS data)
			for(var i = 0; i < noOfData; i++) {
				//log("theContext: l "  + data[i].data());
				//log("theContext: dataColor = " + data[i].style.dataColor());
				...
				if(data[i].dataType() == 0) {
					...
					d.map(function(dd, index) {
						...
						context.select("#theContextObjectContainer")
							.append('circle')
			...
			//log("theContext: th start date :" + startDate);
			//log("theContext: th Stop date :" + lastDate);

			theBrush = d3.svg.brush()
									.x(xContext)
									.extent([startDate, lastDate])
									.on('brush', theContext.brushed);
			...
			function calculateTimeRange(data){
				//log("theContext: calculating time range");
				var start, end;
				if(data.length >= 1) {
					start = data[0].data();
					start = start[0].timestamp - 0;
					//log("theContext: start: " + start);
				...
				if(data.length > 1) {						
					...
					//log("theContext:  timelinehandler: Start timestamp: " + start + " and End timestamp: " + end );
				}
			}
	...
	theContext.contextHeight = function (_) {
		if (!arguments.length)
	...
	theContext.areaSpace = function (_) {
		if (!arguments.length)
			return areaSpace;
		areaSpace = _;
		return theContext;
	...
	//##############################################
	theContext.timelineBarHeight = function (_) {
		if (!arguments.length)
	...
	theContext.brushed = function() {
		//### when the time-range handles are moved
		
		//log("theContext:  $$$$$$$$$$$$$$$$$$$$$$$$ brushed $$$$$$$$$$$$$$$$$$$$$$$$")
		//log("theContext:  extent: " + theBrush.extent());
		...
		subject.notify(theRange, "theContext", scaleVal);//notifying all the observers about the change in range
	};
	
	//#######################################################
	theContext.reload = function(data) {
	...
	theContext.update = function (range, caller, zoomScale, index) {
		//log(zoomScale + " : " + index)
		...
		//log("theContext:  caller = " + caller);
		//log("theContext:  theBrush : " + theBrush.extent());
		//log("theContext:  before " + theBrush.extent());
		theRange = range;
		//theBrush.extent(range);
		//log("theContext:  range = " + theRange + " ### Caller : " + caller + "###");
		
		//log("theContext:  after  " + theBrush.extent());
		if(theBrush.extent() != null) {
			//timeline.changeHandles();
			if(caller == "focusZoomed") {
				//log("theContext:  called by focus")
				...
				if(zoomScale == 1) {
					...
					//log("theContext:  theStartValue = " + theStartValue + " : " + new Date(theRange[0] - 0).getTime() + " diff: " + (theStartValue - new Date(theRange[0] - 0).getTime()));
					//log("theContext:  theEndValue   = " + theEndValue + " : " + new Date(theRange[1] - 0).getTime() + " diff: " + (new Date(theRange[1] - 0).getTime() - theEndValue));
					...
					if(rangeStart < theStartValue) {
						...
						//log("theContext:  < too small: fixed : theRange[1] : " + theRange[1] + " < diff : " + diff);
					}
				...
				context.select('.brush').call(theBrush.extent(theRange));
				//theContext.brushed();
				subject.notify(theRange, "theContextZoomed", zoomScale)
			}
			else {
	...
	theContext.hideData = function(index) {
		log("theContext: about to hide data" + index);
		context.selectAll(".data" + index).attr("visibility", "hidden");
	};
	
	theContext.showData = function(index) {
		log("theContext: about to show data" + index);
		context.selectAll(".data" + index).attr("visibility", "visible");
	};
	
	theContext.changeColor = function(dataToChangeIndex, color) {
